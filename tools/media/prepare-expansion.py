...
def video(item):
 ...
 run(name+'-preview.mp4',['-ss',str(start),'-i',str(OUT/(name+'-film.mp4')),'-t',str(min(6,duration-start)),'-an','-vf','scale='+small,'-c:v','libx264','-crf','25','-preset','fast','-threads','2','-pix_fmt','yuv420p','-movflags','+faststart','-map_metadata','-1'])
