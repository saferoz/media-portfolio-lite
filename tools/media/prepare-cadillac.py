...
def main():
 ...
 run('cadillac-escalade-v2-poster.webp',['-ss',1,'-i',SOURCE,'-frames:v',1,'-vf','scale=720:1280','-c:v','libwebp','-quality',86])
 manifest={'primary':{'source':SOURCE,'sourceSeconds':13.59,'encodedSeconds':13.55,'previewStart':3,'previewSeconds':6,'posterTime':1,'filmAudio':True,'previewAudio':False},'second':{'source':'D:/Downloads/Cadillac.mp4','film':'cadillac-film.mp4','seconds':101.08},'files':[{'name':p.name,'bytes':p.stat().st_size} for p in OUT.glob('cadillac-escalade-v2-*')]}
