  ...
  return <PortfolioRuntime>
    ...
    <main className="project-page">
      ...
      <section className={`project-films page-width ${education ? 'education-project-film' : 'film-pair'}`} id="project-films" aria-label={education ? 'Featured episode' : 'YouTube / Long-form interviews'}>
        ...
        {education && <div className="project-film-intro" data-reveal><h2>5 Hazardous Attitudes</h2><p>The featured episode from the educational series.</p><p>Script, direction, filming, editing and audio finishing by Raden Hanifa.</p></div>}
      </section>
      <section className="production-story page-width" aria-labelledby="production-heading">
        <div className="story-heading" data-reveal><h2 id="production-heading">How I made<br /><span>the series.</span></h2><p>{education ? 'I handled the scripts, direction, filming, editing and audio finishing.' : 'I handled the script, direction, shooting and edit for both episodes, including the motion-graphics titles and dialogue polishing.'}</p></div>
        <div className="process-columns">
          <div data-reveal><h3>Before the shoot</h3><p>{education ? 'Researching the topics, writing the scripts and planning the filming with students and crew.' : 'Developing the script and interview structure, then planning the shots and visual presentation.'}</p></div>
          <div data-reveal><h3>On set</h3><p>{education ? 'Directing the scenes, arranging the set and filming in classroom and simulator environments.' : 'Directing and filming the conversations, with attention to framing, lighting and recorded dialogue.'}</p></div>
          <div data-reveal><h3>In the edit</h3><p>{education ? 'Editing each lesson and cleaning up the recorded dialogue.' : 'Building the story and pacing, creating motion-graphics titles, and polishing the voice audio for the finished episodes.'}</p></div>
        </div>
      </section>
      <section className="project-bts page-width" aria-labelledby="bts-heading">
        <div className="grading-gallery-heading" data-reveal><h2 id="bts-heading">{education ? 'Behind the series.' : 'On set with the students.'}</h2><p>{education ? 'The planning document, lighting and audio setup, and simulator filming from the educational series.' : 'Behind the scenes of Get to Know Our Students.'}</p></div>
      ...
      <section className="grading-close page-width"><h2 data-reveal>Want to see more?</h2><a className="text-link" href={portfolioRequestHref}>Request full portfolio <ArrowUpRightIcon size={20} /></a><Link className="grading-back" href="/#work"><ArrowLeftIcon size={17} /> Back to selected work</Link></section>
    </main>
  </PortfolioRuntime>;
}
