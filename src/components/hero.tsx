...
export function Hero() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => setSource(matchMedia('(max-width: 767px)').matches ? '/media/hero-mobile.mp4' : '/media/hero-desktop.mp4'), 350);
  ...
  return (
    <section className="hero" id="top" ref={container} aria-labelledby="hero-heading">
      ...
      <motion.div className="hero-media" style={reducedMotion ? undefined : { transform }}>
        <Image src="/media/hero-poster.webp" alt="A cinematic frame from SwiftSoft, an AI film by Raden Hanifa" fill priority sizes="100vw" className="hero-poster" />
      ...
      <motion.div className="hero-content page-width" style={reducedMotion ? undefined : { y: contentY, opacity: contentOpacity }}>
        ...
        <button className="hero-feature" onClick={event => openFilm(projects[0], event.currentTarget)} aria-label="Play SwiftSoft film">
