...
function ProjectCard({ project, featured = false }: { project: Project; featured?: boolean }) {
  ...
  const image = <>
    {!imageFailed && <Image src={project.poster} alt={project.placeholder ? `${project.description} — placeholder image, film selection pending` : 'SwiftSoft — cinematic AI film'} fill sizes={featured ? '(max-width: 767px) 100vw, 90vw' : '(max-width: 600px) 50vw, 40vw'} onError={() => setImageFailed(true)} />}
    ...
    {project.placeholder ? <span className="placeholder-label">Preview image · film to come</span> : <>
      <span className="project-film-label">AI filmmaking <span>00:55</span></span>
  ...
  return <article className={`project-card ${featured ? 'featured-card' : ''} ${project.aspect === 'portrait' ? 'portrait-card' : ''} ${project.placeholder ? 'placeholder-card' : ''}`} ref={cardRef} data-project={project.id}>
    ...
    <div className="project-caption"><div><h3>{project.title}</h3><p>{project.placeholder ? project.description : project.contribution}</p></div><span className="project-category">{project.placeholder ? project.category : 'AI film'}</span></div>
  </article>;
}
  ...
  return <section id="work" className="work-section page-width" aria-labelledby="work-heading">
    ...
    <div className={`work-grid ${filter !== 'All work' ? 'is-filtered' : ''}`}>
      {filter === 'All work' ? <>
        ...
        <div className="reels-pair">{projects.filter(project => project.category === 'Reels').map(project => <ProjectCard key={project.id} project={project} />)}</div>
        <div className="landscape-pair">{projects.filter(project => project.category === 'Films' || project.category === 'Motion').map(project => <ProjectCard key={project.id} project={project} />)}</div>
      </> : filtered.map(project => <ProjectCard key={project.id} project={project} featured={!project.placeholder} />)}
    </div>
    <p className="work-footnote">More edits are being selected. SwiftSoft is ready to watch.</p>
  </section>;
}
