...
`tools/media/prepare-hero.py` currently rerenders mobile only and retains the verified desktop manifest and assets. All source shots are four seconds. Desktop overlaps by 0.35s at 3.65, 7.30 and 10.95s (14.96s encoded). Mobile overlaps by 0.35s into shot 2 and 0.15s into shot 3, then cuts directly into shot 4: timeline starts 0, 3.65, 7.50 and 11.50s; 15.50s total. The table below retains desktop start times; mobile uses those independent starts. Both are silent H.264/yuv420p with faststart. `tools/media/hero-manifest.json` records exact source paths, crop fractions, dimensions, timelines, output names and byte counts.
...
| 7.30s | `E:/V1-0014_Rakan Brolls149555201.mov`, 8-12s | `E:/Gen AI Projects/Spiderman-v2/hf_20260803_221432_eb56c480-8903-4090-839a-47ccf212ee14.mp4`, 3-7s |
| 10.95s | `public/media/diriyah-film.mp4`, 7-11s; crop 1920:816:0:132, then fill 16:9 | Existing ARCHI/Eating shot: `E:/TEMPP DELL optionss/Eating Final.mp4`, 3-7s |

Output: `hero-v4-desktop.mp4` (1280×720, 3,000,530 bytes, budget 3,500,000), `hero-v5-mobile.mp4` (540×960, 1,673,600 bytes, budget 1,800,000). Matching `hero-v4-desktop-poster.webp` and `hero-v5-mobile-poster.webp` images are extracted from each encoded opening frame. Every live hero reference uses this version so old cached ARCHI/personal exports cannot act as loading posters. The mobile breakpoint is 767px. No CSS framing/headline changes were made.
...
- Motion: retain the existing hero depth; use a full transform string for hero content. Project cards reveal once with a 750ms mask/translation and 60ms stagger within rows. Chapter headings reveal once (450ms); About copy and contact heading use the existing reveal system. Reel expansion uses a 260ms transform animation; Reel changes use a 280ms full-height vertical slide, with a temporary outgoing canvas still and one incoming video source. Side navigation dims to 18% when inactive, except for visible keyboard focus or fine-pointer hover. Keyboard navigation and reduced motion bypass these effects. Existing Motion/WAAPI/CSS and Lenis suffice; no extra animation library was added.
