...
- Motion: retain the existing hero depth; use a full transform string for hero content. Project frames and captions stay fixed. Only their images settle from scale 1.025 to 1 over 600ms, without delays, masks or opacity changes. Headings enter over 350ms with a 12px translation; About copy and contact heading use the existing reveal system. Reel expansion uses a 260ms transform animation; Reel changes use a 280ms full-height vertical slide, with a temporary outgoing canvas still and one incoming video source. Side navigation dims to 18% when inactive, except for visible keyboard focus or fine-pointer hover. Keyboard navigation and reduced motion bypass these effects. Existing Motion/WAAPI/CSS and Lenis suffice; no extra animation library was added.
