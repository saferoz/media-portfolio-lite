...
Output: `hero-v4-desktop.mp4` (1280×720, 3,000,530 bytes, budget 3,500,000), `hero-v4-mobile.mp4` (540×960, 1,624,310 bytes, budget 1,800,000). Matching `hero-v4-*-poster.webp` images are extracted from each encoded opening frame. Every live hero reference uses this version so old cached ARCHI/personal exports cannot act as loading posters. The mobile breakpoint is 767px. No CSS framing/headline changes were made.
...
- Motion: retain the existing hero depth; use a full transform string for hero content. Chapter headings reveal once (450ms); About copy and contact heading use the existing reveal system. Reel expansion uses a 260ms transform animation; Reel changes use 220ms transform/opacity. Keyboard navigation and reduced motion bypass these effects. Existing Motion/WAAPI/CSS and Lenis suffice; no extra animation library was added.
