...
test('hero uses one bounded rendition without project credit boxes', async ({ page }) => {
  const media: string[] = [];
  page.on('request', request => { if (/hero-v[46]-(desktop|mobile)\.mp4/.test(request.url())) media.push(request.url()); });
  await page.goto('/');
  ...
  expect(media.some(url => url.includes('hero-v6-mobile.mp4'))).toBe(false);
  expect(statSync('public/media/hero-v4-desktop.mp4').size).toBeLessThanOrEqual(3_500_000);
  expect(statSync('public/media/hero-v6-mobile.mp4').size).toBeLessThanOrEqual(1_800_000);
});
