...
const results = await page.evaluate(() => ({ ...window.__portfolioMetrics, resources: performance.getEntriesByType('resource').map(entry => ({ name: entry.name.split('/').pop(), transferSize: entry.transferSize, duration: Math.round(entry.duration) })), fullFilmFetched: performance.getEntriesByType('resource').some(entry => /-film\.mp4(?:$|\?)/.test(entry.name)) }));
